...
1. Pick file (system document picker, both first-launch and Settings paths [PRD]).
   - **Preflight — implemented 2026-09-15 (Task 10.3.1):** as soon as a file is picked, `ArchiveInspecting.preflight` checks its **structure only** — magic, envelope version, crypto suite, field lengths, and the iteration ceiling — with no passphrase, no key derivation and no decryption. A file that is not an export, is truncated, or asks for too many iterations is refused *before* the person is asked for a passphrase. Preflight deliberately does **not** act on the header's `schemaVersion`: that field is only trustworthy once decryption has authenticated it, so a newer-schema export passes preflight and is refused at step 4 [PRD order]. A picked file larger than 64 MB is refused before it is read (an engineering bound — a real export is a few megabytes).
2. Parse envelope header. Unknown magic → "That file isn't a Stabilyz backup." Unknown envelope version or crypto suite → "This backup format isn't supported by this version of Stabilyz." Nothing touched.
   - **Iteration ceiling (Task 10.3.1):** a header asking for more than `ArchiveFormat.maximumIterations` (5,000,000) PBKDF2 iterations is refused here, **before any key derivation**, as `schemaCompatibility(.unsupportedIterationCount)` — not as a damaged file, because the likeliest source is a newer build that raised the ceiling. Copy: *"This backup uses security settings this version of Stabilyz can't open. Update the app, then try restoring again. Your existing data hasn't changed."* A count of zero is not something any writer produces and is reported as damaged. A file that cannot be read at all: *"That file couldn't be opened. Choose it again, or pick a different backup. Your existing data hasn't changed."*
...
2. Snapshot the current store — **serialized, not copied** (Task 10.3.5). Every row, invalid sessions included, is read as `StoreContents` and written in the store's own row format (the columns `EntityMapping` produces, read back through `EntityMapping`), with a **SHA-256 digest over the body** so a truncated, edited or damaged snapshot is refused rather than read. It is written atomically under complete data protection to `Application Support/RestoreStaging`, followed by a restore-in-progress marker, both **before the first write**. Not a copy of the SQLite files: a row snapshot can be verified by digest and compared row-for-row with the store, which is how both the rollback and launch recovery prove they put it back.
...
4. Catastrophe net: a marker found at the next launch means the process ended between staging and the restore's verified end. `RestoreRecoveryService`, run by the router before its first store read, writes the snapshot back, verifies it, and only then removes marker and snapshot. The snapshot is written before the marker and removed after it, so a marker always has a complete snapshot beside it. A damaged snapshot is discarded and logged (the store is left as its last single-transaction save left it); an unreadable one (e.g. a locked device) or a failed write-back keeps the staging for the next launch. A restore clears its staging only once its outcome is verified; one whose store could not be put back keeps it, and a later restore never overwrites that original snapshot.
